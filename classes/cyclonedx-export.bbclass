...
# Path to an additional SBOM file generated by the recipe (e.g., Maven CycloneDX output)
# Useful for recipes that build dependencies not tracked by Yocto (e.g., Java libraries)
CYCLONEDX_ADDITIONAL_SBOM ??= ""

# Optional: Set this to "1" to warn if CYCLONEDX_ADDITIONAL_SBOM is referenced but file doesn't exist
CYCLONEDX_ADDITIONAL_SBOM_STRICT ??= "0"
...
python do_populate_cyclonedx() {
    ...
    # Add additional SBOM data if present
    additional_sbom_path = d.getVar("CYCLONEDX_ADDITIONAL_SBOM")
    if additional_sbom_path:
        bb.debug(1, f"Processing CYCLONEDX_ADDITIONAL_SBOM for recipe {pn}")
        pn_list = merge_additional_sbom(d, pn_list, additional_sbom_path)
...
def merge_additional_sbom(d, pn_list, additional_sbom_path):
    """
    Merge components and dependencies from an additional SBOM file
    (e.g., generated by Maven CycloneDX plugin) into the recipe's partial SBOM.
    
    Args:
        d: BitBake data
        pn_list: The recipe's partial SBOM data
        additional_sbom_path: Path to the additional SBOM file
    
    Returns:
        Updated pn_list with merged components and dependencies
    """
    if not additional_sbom_path or not os.path.exists(additional_sbom_path):
        strict_mode = d.getVar("CYCLONEDX_ADDITIONAL_SBOM_STRICT") == "1"
        if additional_sbom_path and strict_mode:
            bb.error(f"CYCLONEDX_ADDITIONAL_SBOM file not found: {additional_sbom_path}")
        elif additional_sbom_path:
            bb.debug(1, f"CYCLONEDX_ADDITIONAL_SBOM file not found, skipping: {additional_sbom_path}")
        return pn_list
    
    try:
        bb.debug(2, f"Loading additional SBOM from {additional_sbom_path}")
        additional_sbom = read_json(additional_sbom_path)

        # Track which components we're adding to avoid duplicates based on purl/cpe
        existing_purls = {pkg.get("purl") for pkg in pn_list["pkgs"] if pkg.get("purl")}
        existing_cpes = {pkg.get("cpe") for pkg in pn_list["pkgs"] if pkg.get("cpe")}

        pn_list.setdefault("pkgs", [])

        # Merge main component from metadata
        if "metadata" in additional_sbom and "component" in additional_sbom["metadata"]:
            add_additional_component(d, pn_list, additional_sbom["metadata"]["component"], existing_purls, existing_cpes)

        # Merge components
        if "components" in additional_sbom and additional_sbom["components"]:
            for component in additional_sbom["components"]:
                add_additional_component(d, pn_list, component, existing_purls, existing_cpes)
            
            bb.debug(2, f"Merged {len(additional_sbom['components'])} components from additional SBOM")
        
        # Merge dependencies
        if "dependencies" in additional_sbom and additional_sbom["dependencies"]:
            pn_list.setdefault("dependencies", [])
            for dependency in additional_sbom["dependencies"]:
                if dependency not in pn_list["dependencies"]:
                    pn_list["dependencies"].append(dependency)
            bb.debug(2, f"Merged {len(additional_sbom['dependencies'])} dependencies from additional SBOM")
        
        # Merge vulnerabilities (if present)
        if "vulnerabilities" in additional_sbom and additional_sbom["vulnerabilities"]:
            pn_list.setdefault("cves", [])
            # CVEs should have unique IDs, so check for duplicates
            existing_cve_ids = {cve.get("id") for cve in pn_list["cves"]}
            for vulnerability in additional_sbom["vulnerabilities"]:
                if vulnerability.get("id") not in existing_cve_ids:
                    pn_list["cves"].append(vulnerability)
                    existing_cve_ids.add(vulnerability.get("id"))
            bb.debug(2, f"Merged {len(additional_sbom['vulnerabilities'])} vulnerabilities from additional SBOM")
        
        return pn_list
    
    except Exception as e:
        bb.warn(f"Failed to merge additional SBOM from {additional_sbom_path}: {e}")
        return pn_list

def add_additional_component(d, pn_list, component, existing_purls, existing_cpes):
    """
    Add a single component to the pn_list. 
    """

    # Generate CPE if missing (optional, for consistency)
    if "cpe" not in component and "purl" in component:
        # Extract vendor/product from purl if possible
        # Format: pkg:type/namespace/name@version
        purl = component["purl"]
        try:
            # Simple extraction: pkg:generic/vendor/product@version
            parts = purl.split("/")
            if len(parts) >= 3:
                vendor = parts[-2]
                product = parts[-1].split("@")[0]
                version = component.get("version", "unknown")
                component["cpe"] = f'cpe:2.3:*:{vendor}:{product}:{version}:*:*:*:*:*:*:*'
        except:
            pass  # If parsing fails, just continue without CPE
            
    # Skip if component already exists (based on purl or cpe)
    if component.get("purl") in existing_purls or component.get("cpe") in existing_cpes:
        bb.debug(2, f"Skipping duplicate component: {component.get('name', 'unknown')}")
        return
                
    # Add missing bom-ref if not present
    if "bom-ref" not in component:
        import uuid
        component["bom-ref"] = str(uuid.uuid4())
                
    pn_list["pkgs"].append(component)
    existing_purls.add(component.get("purl"))
    existing_cpes.add(component.get("cpe")) 
